...
# tabela LEDs para conversão (0 a F)
...
scan_inicio:
    ...
    sb   $t0, 0($t8)
    lb   $t1, 0($t9)
    andi $t2, $t1, 0xF0      # MASCARA: ignora bits de linha (ruído), pega só coluna
...
achou_L1:
    li   $v0, 0              # Base numérica da linha 1
    move $t1, $t2            # Move o valor mascarado para t1
    j    calcula_final

achou_L2:
    li   $v0, 4              # Base numérica da linha 2
    move $t1, $t2
    j    calcula_final

achou_L3:
    li   $v0, 8              # Base numérica da linha 3
    move $t1, $t2
    j    calcula_final
